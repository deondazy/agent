...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            app_env=os.getenv("DENOSYSBOT_APP_ENV", "development"),
            api_host=os.getenv("DENOSYSBOT_API_HOST", "0.0.0.0"),
            api_port=int(os.getenv("DENOSYSBOT_API_PORT", "8000")),
            redis_url=os.getenv("DENOSYSBOT_REDIS_URL", "redis://localhost:6379/0"),
            database_url=os.getenv(
                "DENOSYSBOT_DATABASE_URL",
                "postgresql+psycopg://postgres:postgres@localhost:5432/denosysbot",
            ),
            model_fallback_order=parsed_fallback,
            model_openai_model=os.getenv("DENOSYSBOT_MODEL_OPENAI_MODEL", "gpt-4.1"),
            model_openai_base_url=os.getenv("DENOSYSBOT_MODEL_OPENAI_BASE_URL", "https://api.openai.com"),
            model_openai_timeout_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_OPENAI_TIMEOUT_SECONDS", "60.0")
            ),
            model_openai_max_retries=int(os.getenv("DENOSYSBOT_MODEL_OPENAI_MAX_RETRIES", "2")),
            model_openai_initial_backoff_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_OPENAI_INITIAL_BACKOFF_SECONDS", "0.5")
            ),
            model_openai_max_backoff_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_OPENAI_MAX_BACKOFF_SECONDS", "4.0")
            ),
            model_anthropic_model=os.getenv("DENOSYSBOT_MODEL_ANTHROPIC_MODEL", "claude-sonnet-4-5"),
            model_anthropic_base_url=os.getenv(
                "DENOSYSBOT_MODEL_ANTHROPIC_BASE_URL",
                "https://api.anthropic.com",
            ),
            model_anthropic_version=os.getenv(
                "DENOSYSBOT_MODEL_ANTHROPIC_VERSION",
                "2023-06-01",
            ),
            model_anthropic_timeout_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_ANTHROPIC_TIMEOUT_SECONDS", "60.0")
            ),
            model_anthropic_max_retries=int(
                os.getenv("DENOSYSBOT_MODEL_ANTHROPIC_MAX_RETRIES", "2")
            ),
            model_anthropic_initial_backoff_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_ANTHROPIC_INITIAL_BACKOFF_SECONDS", "0.5")
            ),
            ...
            model_ollama_base_url=os.getenv(
                "DENOSYSBOT_MODEL_OLLAMA_BASE_URL",
                "http://localhost:11434",
            ),
            model_ollama_timeout_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_OLLAMA_TIMEOUT_SECONDS", "90.0")
            ),
            model_ollama_max_retries=int(os.getenv("DENOSYSBOT_MODEL_OLLAMA_MAX_RETRIES", "2")),
            model_ollama_initial_backoff_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_OLLAMA_INITIAL_BACKOFF_SECONDS", "0.5")
            ),
            model_ollama_max_backoff_seconds=float(
                os.getenv("DENOSYSBOT_MODEL_OLLAMA_MAX_BACKOFF_SECONDS", "4.0")
            ),
            vm_provider=os.getenv("DENOSYSBOT_VM_PROVIDER", "disposable-vm"),
        )
